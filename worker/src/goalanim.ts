...
Rules: Modern Standard Arabic as Arabic sports TV commentators speak, energetic but factual, natural for text-to-speech (no abbreviations, no Latin letters — transliterate every name into Arabic, e.g. Cherki → شيركي, Bouaddi → بوعدي). voices[i] narrates storyboard beat i (9 beats: hook, 1-7, CTA), 8-13 words each — the whole narration must fit in 45 seconds, so one short sentence per beat, no filler, no repetition of what another beat already said; voices[0] must end with "شاهد كيف جاء" and voices[8] must say "تابع صفحة بريسينغ تسعين". captions[i] = [small, big]: small = context, max 6 words; big = the punch, max 4 words (it is displayed in a large calligraphic font). tags = the 6 labels written on the numbered steps, ARABIC ONLY, 1-2 words each, never a Latin letter (they are drawn on an Arabic pitch diagram); in this order: the opening or recovery, the scorer's run, the decoy movements, the arrival in the shooting position, the assist, the finish. cover = max 5 words for the closing card. title = max 60 characters for the video title. post = 4 lines for the Facebook caption: (1) a question about the goal whose FIRST words are the scorer's Arabic name (scorerAr), (2) one sentence on what made it (assist, distance, minute), (3) a closed question to the reader ("هل كان الحارس مخطئاً؟", "أي لمسة صنعت الفارق؟"), (4) exactly "تابع بريسينغ 90 لتحليل هدف كل يوم ⚽" — plus one line of 5 hashtags (mix Arabic and English, include #Pressing90). No emoji anywhere except the post. Never invent statistics beyond the storyboard; the distance in metres and the score are facts you may use. Digits are fine (e.g. "الدقيقة 32").`
  ...
  // 45-55 s reels (Mehdi, 2026-09-19): the length of the video IS the length of the narration, so a line that runs long
  // is cut back at its last sentence boundary rather than stretching the whole reel.
  const trimVoice = (v: string) => { const t = clean(v, 260); if (t.length <= 150) return t; const cut = t.slice(0, 150); const dot = Math.max(cut.lastIndexOf('.'), cut.lastIndexOf('،'), cut.lastIndexOf('؟')); return (dot > 60 ? cut.slice(0, dot + 1) : cut.slice(0, cut.lastIndexOf(' '))).trim() }
  const voices = (Array.isArray(j.voices) ? j.voices : []).map(trimVoice).filter(Boolean)
  ...
  const tagsIn = (Array.isArray(j.tags) ? j.tags : []).map((t) => clean(t, 18)).filter(Boolean)
  if (voices.length !== 9 || captions.length !== 9 || tagsIn.length < 6) throw new Error(`incomplete texts (${voices.length} voices, ${captions.length} captions, ${tagsIn.length} tags)`)
  const t = templateTexts(mt)
  // Step labels must be Arabic (Brentford, 2026-09-19): the model read the list of steps in the prompt as the labels
  // themselves and drew "run", "opening", "decoys" on an Arabic diagram. Any Latin letter falls back to the template label.
  const tags = t.tags.map((fallback, i) => (tagsIn[i] && !/[A-Za-z]/.test(tagsIn[i]) ? tagsIn[i] : fallback))
...
function templateTexts(mt: Meta): Texts {
  ...
  return { scorerAr, assistAr, voices, captions, tags: ['الاستخلاص', 'الانطلاقة', 'التمويه', 'الوصول', 'التمريرة', 'الإنهاء'], cover: `هدف ${S} · ${mt.distance} متراً`, title: `كيف جاء هدف ${S}؟ | ${mt.scoreLine}`, post: `${S}: كيف جاء هذا الهدف؟ ⚽\n${A ? `${A} يصنع، ` : ''}${S} يُنهي من ${mt.distance} متراً. ${mt.scoreLine}.\nهل كان بإمكان الدفاع إيقافه؟\nتابع بريسينغ 90 لتحليل هدف كل يوم ⚽\n#تحليل_الأهداف #Pressing90 #${mt.team.replace(/\s+/g, '')} #${S}` }
}
