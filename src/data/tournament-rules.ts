 ...
 *
 * Front-end mirror of worker/src/wc26-data/tournament-rules.ts. The
 * worker uses its copy to power the AI assistant's `get_tournament_rules`
 * tool; the front-end uses this copy to render /rules. Keeping the two
 * in sync is a manual step — when FIFA amends a rule, edit both files.
 *
 * Why two copies? Workers don't share a bundle with the SPA, and the
 * file is tiny + changes on the order of years. A shared package would
 * cost more in build-config than it saves.
 */
